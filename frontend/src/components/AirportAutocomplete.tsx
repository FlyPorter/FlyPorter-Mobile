...
export default function AirportAutocomplete({
  ...
  useEffect(() => {
    ...
    if (searchTerm.length > 0) {
      ...
      if (isEditing && filtered.length > 0 && !isAirportFormat(value)) {
      ...
      {compact && value && !isEditing ? (
        ...
        <View style={styles.inputContainer}>
          <TextInput
            ...
            onFocus={() => {
              setIsEditing(true);
              // Only show suggestions if there's already text and it doesn't match airport format
              const searchTerm = getSearchTerm(value).trim();
              if (searchTerm.length > 0 && !isAirportFormat(value)) {
                // Suggestions will be shown by useEffect
              }
            }}
            onBlur={() => {
              // Delay hiding suggestions to allow onPress to fire
              setTimeout(() => {
                if (!isAirportFormat(value) || !value) {
                  setIsEditing(false);
                }
                setShowSuggestions(false);
              }, 200);
